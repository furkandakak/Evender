package com.furkandakak.together
...
import com.furkandakak.together.databinding.ActivityMainPageDetailBinding
...
class MainPageDetail : AppCompatActivity() {
    ...
    private lateinit var binding: ActivityMainPageDetailBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityMainPageDetailBinding.inflate(layoutInflater)
        ...
        //get doc id for add user join or favorite array
    ...
    private fun getDataForChat() {
        firestore.collection("Messages").whereEqualTo("documentId", get_doc_id_for_selected_event)
            .orderBy(
                "dateForSorting",
    ...
    fun add_to_fav(view: View) {
       //add to firebase
        if (auth.currentUser != null) {
            ...
            Toast.makeText(applicationContext, "Favoriye Eklendi", Toast.LENGTH_SHORT).show()
            this.recreate()

            //add user's id to firebase
            firestore.collection("Created").document(get_doc_id_for_selected_event)
                .update("favoriteUser", FieldValue.arrayUnion(now_user))
        }
    }

    //add join when clicked
    fun add_to_join(view: View) {
        if (auth.currentUser != null) {
            ...
            //add user's id to firebase
            firestore.collection("Created").document(get_doc_id_for_selected_event)
                .update("joinedUser", FieldValue.arrayUnion(now_user))
        }
    }
