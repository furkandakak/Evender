...
import com.furkandakak.together.databinding.ActivityFavoriteEventDetailBinding
...
class FavoriteEventDetail : AppCompatActivity()
{
    ...
    private lateinit var binding: ActivityFavoriteEventDetailBinding
    lateinit var get_doc_id_for_selected_event: String


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityFavoriteEventDetailBinding.inflate(layoutInflater)
        ...
        //get doc id to add favorite or join
    ...
    //delete from favorite list when clicked
    fun remove_from_fav(view: View)
    {
        if (auth.currentUser != null) {
            ...
            //delete users favorite from firebase
            firestore.collection("Created").document( get_doc_id_for_selected_event).update("favoriteUser", FieldValue.arrayRemove(now_user))
            this.recreate()
            ...
            finishAffinity()
            Toast.makeText(applicationContext, "Favorilerimden Çıkarıldı", Toast.LENGTH_SHORT).show()

        }
    }


    //add  to joined when clicked
    fun add_to_join(view: View)
    {
        if (auth.currentUser != null) {
            ...
            firestore.collection("Created").document( get_doc_id_for_selected_event).update("joinedUser", FieldValue.arrayUnion(now_user))
            this.recreate()
            Toast.makeText(applicationContext, "Katıldıklarıma Eklendi", Toast.LENGTH_SHORT).show()
    ...
    fun click_for_message(view: View)
    {
        ...
        send.setOnClickListener {
            ...
            //burada REGEX ile sadece boşluk bırakılmasını engelliyorum
            var re = Regex("^\\s*$")
            //butona basılınca boş bırakılan yer var mı diye kontrol ediyorum
